...
#define SAYit_(lvl, fmt, ...)                                       \
    do {                                                            \
        if (DEBUG_LEVEL >= (lvl))                                   \
            fprintf(stderr, fmt "%s", __FILE__, __LINE__, __func__, \
                    __VA_ARGS__);                                   \
    } while (0)
...
#define fmt(ts)                                                            \
    fmt_(((char[((1 << WHEEL_BIT) *WHEEL_NUM) + WHEEL_NUM + 1]){0}), (ts), \
         WHEEL_BIT, WHEEL_NUM)
...
#define bin64(ts) \
    bin64_(((char[((1 << WHEEL_BIT) *WHEEL_NUM) + 1]){0}), (ts), WHEEL_BIT)
