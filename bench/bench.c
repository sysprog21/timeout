...
static int bench_add(lua_State *L)
{
    ...
    i = (lua_isnoneornil(L, 2)) ? random() % B->count
                                : (unsigned) luaL_checkinteger(L, 2);
    t = (lua_isnoneornil(L, 3)) ? random() % B->timeout_max
...
static const luaL_Reg bench_metatable[] = {{"__pairs", &bench__pairs},
                                           {"__gc", &bench__gc},
                                           {NULL, NULL}};

static const luaL_Reg bench_globals[] = {{"new", &bench_new},
                                         {"clock", &bench_clock},
                                         {NULL, NULL}};
